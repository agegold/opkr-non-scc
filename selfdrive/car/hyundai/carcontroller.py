...
class CarController():
  ...
  def update(self, c, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert,
    ...
    # print debug data
    trace1.printf1("AR?>" + str(CS.opkr_autoresume) + " Rs?>" + "{:.1f}".format(reenable_cruise_atspd) + " DS>" + "{:.1f}".format(desired_speed) + " CCr>" + "{:.1f}".format(CS.current_cruise_speed) + " StP>" + "{:.2f}".format(stoplinesp) + " DSpd>" + "{:.1f}".format(l0v_distval_mph) + " DSpM>" + "{:.1f}".format(lead_vdiff_mph) + " Conf>" + "{:.2f}".format(overall_confidence))
